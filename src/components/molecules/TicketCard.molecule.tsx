...
const TicketCard: React.FC<Props> = ({disabled}) => {
  return (
    <div className="flex flex-col gap-0.5 w-full">
      <div className={`flex flex-row justify-between items-center bg-primary-purple-101 dark:bg-primary-purple-109 p-2 gap-2 ${disabled ? "!bg-neutral-400" : ''}`}>
        <div className="flex flex-row items-start justify-start gap-2 w-11/12 border-r-2 pr-3 border-primary-purple-105 border-dashed">
          ...
          <div className="flex flex-col w-2/3 gap-1 ">
            ...
            <Typography weight="bold" variant="text-lg" className={`truncate ${disabled ? "" : "!text-primary-purple-105"}`}>Decentralized with Portalis</Typography>
            ...
            <div className="flex flex-row items-center gap-2">
              ...
              <Typography weight="light" variant="text-xs" className="truncate">Jakarta Convention Center</Typography>
            </div>
          </div>
        </div>
        <Link href={'/ticket/1'} className="w-1/12 h-auto">
          <HiChevronDoubleRight className="!text-primary-purple-105 w-full" />
        </Link>
      </div>
